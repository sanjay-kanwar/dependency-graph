...
const buildQuarter = (teams: TeamNode[]) => {
  ...
  for (let i = 0; i < teams.length; i++) {
    for (let j = 0; j < QUARTERS.length; j++) {
      ...
      const { subNodes, edges } = buildServicesNode(quarterNode);
...
const buildServicesNode = (quarter: QuarterNode) => {
  const subNodes: QuarterNode[] = [];
  const edges: Edge[] = [];
  const center = { x: window.innerWidth / 2, y: window.innerHeight / 2 };
  for (let i = 0; i < SERVICES.length; i++) {
    const degrees = i * (360 / 50);
    const radians = degrees * (Math.PI / 180);
    const x = 250 * Math.cos(radians) + center.x;
    const y = 250 * Math.sin(radians) + center.y;
    const node = {
      id: `${quarter.id}_${SERVICES[i]}`,
      data: { label: `${SERVICES[i]}` },
      position: { x: i === 0 ? -30 : 30, y: 40 },
      parentId: quarter.id,
      ...nodeDefaults,
    };
    const edge = {
      id: `edge-${quarter.id}_${SERVICES[i]}`,
      source: quarter.id,
      target: `${quarter.id}_${SERVICES[i]}`,
      style: {
        color: "red",
      },
    };
